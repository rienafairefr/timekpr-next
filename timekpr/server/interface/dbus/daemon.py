...
class timekprDaemon(dbus.service.Object):
    ...
    def executeTimekprWorker(self):
        ...
        while not self._finishExecution:
            ...
            log.log(
                ...
                "--- perf: avg ela: %s, loadavg: %s, %s, %s ---" % (str(execLen / execCnt), lavg[0], lavg[1], lavg[2]),
            )
    ...
    def checkUsers(self):
        ...
        for rUserName, userDict in userList.items():
            ...
            if not userhelper.isUserValid(userDict[cons.TK_CTRL_UID], userDict[cons.TK_CTRL_UNAME]):
                ...
                log.log(
                    cons.TK_LOG_LEVEL_INFO,
                    'NOTE: system or mismatched user "%s" explicitly excluded' % (rUserName),
                )
        ...
        for rUserName in self._timekprUserList:
            ...
            # adjust time spent
            userActiveEffective, userActiveActual, userScreenLocked = self._timekprUserList[
                rUserName
            ].adjustTimeSpentActual(self._timekprConfig)
            ...
            if self._timekprConfig.getTimekprPlayTimeEnabled():
                ...
                if isPTEnabled and isPTActive:
                    ...
                    if (isPTAccounted and timeLeftPT < 0.0042) or (
                        timeHourUnaccounted
                        and not self._timekprUserList[rUserName].getUserPlayTimeUnaccountedIntervalsEnabled()
                    ):
                        # killing processes
                        self._timekprPlayTimeConfig.killPlayTimeProcesses(self._timekprUserList[rUserName].getUserId())
                    else:
                        # active count
                        timePTActivityCnt = self._timekprPlayTimeConfig.getMatchedUserProcessCnt(
            ...
            if rUserName in self._timekprUserRestrictionList:
                ...
                if (
                    (not userActiveActual and timeLeftToday > self._timekprConfig.getTimekprTerminationTime())
                    or timeHourUnaccounted
                ) and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RESTY] in (
                    cons.TK_CTRL_RES_T,
                    cons.TK_CTRL_RES_D,
                ):
                    log.log(
                        cons.TK_LOG_LEVEL_INFO,
                        'SAVING user "%s" from ending his sessions / shutdown' % (rUserName),
                    )
                    # remove from death list
                    self._timekprUserRestrictionList.pop(rUserName)
                # if restricted time has passed for hard restrictions, we need to lift the restriction
                elif (
                    timeLeftInARow > self._timekprConfig.getTimekprTerminationTime() or timeHourUnaccounted
                ) and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RESTY] in (
                    cons.TK_CTRL_RES_T,
                    cons.TK_CTRL_RES_D,
                ):
                    log.log(
                        cons.TK_LOG_LEVEL_INFO,
                        'RELEASING terminate / shutdown from user "%s"' % (rUserName),
                    )
                ...
                # update restriction stats
                    ...
                    self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] = max(
                        self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] - 1,
                        0,
                    )
                    # only if user is active / screen is not locked
                    if (
                        userActiveActual
                        and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RESTY]
                        in (cons.TK_CTRL_RES_T, cons.TK_CTRL_RES_D)
                    ) or (
                        ...
                        # update active states for restriction routines
                        self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEL] = max(
                            self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEL] - 1,
                            0,
                        )
            ...
            if (
                ...
                self._timekprUserRestrictionList[rUserName] = {
                    ...
                    cons.TK_CTRL_RESTY: self._timekprUserList[
                        rUserName
                    ].getUserLockoutType(),  # restricton type: lock, suspend, suspendwake, terminate, shutdown
                    ...
                    cons.TK_CTRL_USWKU: (
                        self._timekprUserList[rUserName].findNextAvailableIntervalStart()
                        if self._timekprUserList[rUserName].getUserLockoutType() == cons.TK_CTRL_RES_W
    ...
    def _restrictUsers(self):
        ...
        for rUserName in self._timekprUserRestrictionList:
                ...
                if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEL] <= 0:
                    log.log(
                        ...
                        % (
                            ...
                            str(self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_FCNTD]),
                        ),
                    )
                        ...
                        try:
                            ...
                            if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RESTY] == cons.TK_CTRL_RES_T:
                                ...
                                self._timekprLoginManager.terminateUserSessions(
                                    rUserName,
                                    self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_UPATH],
                                    self._timekprConfig,
                                )
                            # shut
            ...
            # we are going to LOCK user sessions
                ...
                if isUserInactive and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] <= 0:
                    ...
                    log.log(
                        cons.TK_LOG_LEVEL_INFO,
                        'time is up, but user "%s" not active, not enforcing the lock' % (rUserName),
                    )
                ...
                # lock must be enforced only if user is active
                    ...
                    if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] <= 0:
                        ...
                        if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_FCNTD] <= 0:
                            ...
                            log.log(
                                cons.TK_LOG_LEVEL_INFO,
                                'time is up for user "%s", enforcing the LOCK' % (rUserName),
                            )
                            # lock computer
            ...
            # we are going to SUSPEND user sessions
                ...
                if isUserInactive and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] <= 0:
                    ...
                    log.log(
                        cons.TK_LOG_LEVEL_INFO,
                        'time is up, but user "%s" not active, not enforcing the suspend' % (rUserName),
                    )
                ...
                # suspend / lock must be enforced only if user is active
                    ...
                    if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_FCNTD] <= 0:
                        ...
                        if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEL] <= 0:
                            ...
                            log.log(
                                cons.TK_LOG_LEVEL_INFO,
                                'time is up for user "%s", enforcing the SUSPEND' % (rUserName),
                            )
                            # set restriction for repetitive lock when suspending
                            self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] = cons.TK_CTRL_LCDEL
                            ...
                            if self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_USWKU] is not None:
                                ...
                                if userhelper.setWakeUpByRTC(
                                    self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_USWKU]
                                ):
                                    log.log(
                                        cons.TK_LOG_LEVEL_INFO,
                        ...
                        # do not enforce lock right away after suspend, wait a little
                            ...
                            > self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEL]
                            > 0
                            and self._timekprUserRestrictionList[rUserName][cons.TK_CTRL_RTDEA] <= 0
                        ):
                            # log suspend lock
                            log.log(
                                cons.TK_LOG_LEVEL_INFO,
        ...
        log.log(
            cons.TK_LOG_LEVEL_INFO,
            "RESTRICTIONS, completed with: %s" % (str(len(self._timekprUserRestrictionList) > 0)),
        )
    ...
    def getUserInformation(self, pUserName, pInfoLvl):
        ...
        try:
            ...
            if pInfoLvl in (cons.TK_CL_INF_FULL, cons.TK_CL_INF_SAVED):
                ...
                # load config
                result, message, userConfigurationStore = userConfigProcessor.getSavedUserInformation(
    ...
    def setPlayTimeUnaccountedIntervalsEnabled(self, pUserName, pPlayTimeUnaccountedIntervalsEnabled):
        ...
        try:
            ...
            # load config
            result, message = userConfigProcessor.checkAndSetPlayTimeUnaccountedIntervalsEnabled(
    ...
    def setTimekprFinalNotificationTime(self, pFinalNotificationTimeSecs):
        ...
        try:
            ...
            # check and set config
            result, message = mainConfigurationProcessor.checkAndSetTimekprFinalNotificationTime(
