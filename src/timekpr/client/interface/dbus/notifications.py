    ...
    def notifyUser(
        ...
        if self._dbusConnections[self.CL_CONN_NOTIF][self.CL_IF] is not None:
            ...
            if (
                ...
                and (datetime.now() - self._lastPTNotifDT).total_seconds()
                >= (
                    notificationTimeout
                    if notificationTimeout > 0
                    else (datetime.now() - self._lastPTNotifDT).total_seconds() + 1
                )
            ):
                self._lastPTNotifId = 0
            elif (
                ...
                and (datetime.now() - self._lastNotifDT).total_seconds()
                >= (
                    notificationTimeout
                    if notificationTimeout > 0
                    else (datetime.now() - self._lastNotifDT).total_seconds() + 1
                )
            ):
                self._lastNotifId = 0
